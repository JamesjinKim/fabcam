...
- **Edge Node (Raspberry Pi + Camera)**:
    - 카메라로부터 영상 스트림을 받아 지정된 형식(예: MP4)으로 녹화합니다. 이때, `picamera2` 라이브러리의 `FfmpegOutput` 기능을 활용하여 FFmpeg를 통해 효율적으로 비디오를 인코딩하고 저장합니다.
    - 녹화된 영상 파일을 실시간 또는 주기적으로 중앙 NAS의 지정된 경로로 전송합니다. (rsync 프로토콜 사용)
...
- **음성 스트림 통합**: `picamera2`의 FFmpeg 백엔드를 활용하여 별도로 캡처된 음성 스트림을 녹화 영상에 결합하는 기능 추가.
