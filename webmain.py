...
# AI 감지 엔진 임포트
from simple_ai_detector import SimpleAIDetector, Detection, draw_detections
...
try:
    ...
    logger.warning("HandDetector not available - MediaPipe not installed")
...
class CameraManager:
    ...
    def __init__(self):
        ...
        self.latest_detections = {}  # 카메라별 최신 감지 결과
        self.ai_stats = {
            'total_detections': 0,
            'detection_rate': 0.0,
        ...
        # 알림 시스템 초기화
        self.notification_system = NotificationSystem(config_manager)

        self._init_ai_detection()

        # 통계 정보
        self.stream_stats = {
            0: {"frame_count": 0, "avg_frame_size": 0, "fps": 0, "last_update": 0, "recording": False, "ai_detections": 0},
            1: {"frame_count": 0, "avg_frame_size": 0, "fps": 0, "last_update": 0, "recording": False, "ai_detections": 0}
        }
        ...
        try:
            while True:
                try:
                    ...
                    # AI 감지 처리
                    detections = self.process_ai_detection(bgr_frame, target_camera)

                    # 감지 결과가 있으면 바운딩박스 그리기
                    if detections and self.ai_enabled:
                        bgr_frame = draw_detections(bgr_frame, detections)

                    # BGR을 RGB로 다시 변환 후 JPEG 인코딩
                    final_frame = cv2.cvtColor(bgr_frame, cv2.COLOR_BGR2RGB)
                    success, frame_data = cv2.imencode('.jpg', final_frame, [cv2.IMWRITE_JPEG_QUALITY, 80])
                    if not success:
                        continue
                    ...
                    if current_time - last_fps_update >= 1.0:
                        ...
                        self.stream_stats[self.current_camera] = {
                            ...
                            "recording": recorder.is_recording if recorder else False,
                            "ai_detections": self.stream_stats[self.current_camera].get("ai_detections", 0)
                        }
    ...
    def _init_ai_detection(self):
        """AI 감지 시스템 초기화"""
        try:
            logger.info("[AI] AI 감지 시스템 초기화 중...")

            # AI 감지 활성화 여부 확인 (설정에서)
            ai_enabled = config_manager.get('ai_detection.enabled', True)
            detection_mode = config_manager.get('ai_detection.detection_mode', 'general')

            if ai_enabled:
                if detection_mode == 'hand' and HAND_DETECTOR_AVAILABLE:
                    # 손 감지 모드 - MediaPipe 사용
                    logger.info("[AI] 손 감지 모드 초기화 (MediaPipe)")
                    hand_config = config_manager.get('ai_detection.hand_detection', {})
                    self.ai_detector = HandDetector(
                        ...
                        max_num_hands=hand_config.get('max_hands', 2)
                    )
                    self.ai_enabled = True
                    self.detection_mode = 'hand'
                    logger.info("[AI] 손 감지 시스템 활성화 완료")
                else:
                    # 일반 객체 감지 모드 - YOLOv5 사용
                    logger.info("[AI] 일반 객체 감지 모드 초기화 (YOLOv5)")
                    confidence_threshold = config_manager.get('ai_detection.confidence_threshold', 0.5)
                    self.ai_detector = SimpleAIDetector(confidence_threshold)

                    if self.ai_detector.initialize():
                        self.ai_detector.start_detection()
                        self.ai_enabled = True
                        self.detection_mode = 'general'
                        logger.info("[AI] 일반 객체 감지 시스템 활성화 완료")
                    else:
                        logger.error("[AI] AI 감지 시스템 초기화 실패")
                        self.ai_enabled = False
            else:
                logger.info("[AI] AI 감지 시스템 비활성화 (설정)")
                self.ai_enabled = False

        except Exception as e:
            logger.error(f"[AI] AI 감지 시스템 초기화 오류: {e}")
            self.ai_enabled = False

    def process_ai_detection(self, frame: np.ndarray, camera_id: int):
        """
        프레임에 대해 AI 감지 수행
        ...
        Args:
            ...
            camera_id: 카메라 ID
        """
        if not self.ai_enabled or not self.ai_detector:
            return []

        try:
            # 5프레임마다 AI 처리 (성능 최적화)
            frame_count = self.stream_stats[camera_id]["frame_count"]
            if frame_count % 5 != 0:
            ...
            # 통계 업데이트
            if detections:
                if "ai_detections" not in self.stream_stats[camera_id]:
                    self.stream_stats[camera_id]["ai_detections"] = 0
                self.stream_stats[camera_id]["ai_detections"] += len(detections)
                self.ai_stats['total_detections'] += len(detections)
                self.ai_stats['last_detection_time'] = time.time()

                # 알림 이벤트 처리
                notification_events = self.notification_system.process_detections(detections, camera_id)

                # 감지 결과 로깅 (주요 객체만)
                for det in detections:
                    if det.confidence > 0.8:  # 높은 신뢰도만 로깅
                        if hasattr(self, 'detection_mode') and self.detection_mode == 'hand':
                            logger.info(f"[HAND-CAM{camera_id}] {det.label}: {det.confidence:.2f}")
                        else:
                            logger.info(f"[AI-CAM{camera_id}] {det.label}: {det.confidence:.2f}")

            return detections

        except Exception as e:
            logger.error(f"[AI] AI 감지 처리 오류: {e}")
            return []
    ...
    def get_ai_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "camera_detections": {
                ...
                for cam_id, stats in self.stream_stats.items()
            }
        }

    def toggle_ai_detection(self, enable: bool) -> bool:
        """AI 감지 활성화/비활성화"""
        try:
            if enable and not self.ai_enabled:
                # AI 감지 활성화
            ...
            logger.error(f"[AI] AI 감지 토글 오류: {e}")
            return False
    ...
    async def shutdown(self):
        ...
        # AI 감지 시스템 정리
        if self.ai_detector:
            logger.info("[AI] AI 감지 시스템 종료 중...")
            try:
                self.ai_detector.cleanup()
            except Exception as e:
                logger.error(f"[AI] AI 감지 시스템 종료 오류: {e}")
